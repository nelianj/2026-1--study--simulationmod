...
annotate!(plt1, maximum(df_ode.t) * 0.7, maximum(df_ode.N) * 0.8, text("Параметры:\nβ = $(p[1])\nc = $(p[2])\nγ = $(p[3])\nR0 = $(round(R0, digits=2))", 8, :left))
...
annotate!(plt2, peak_time, peak_value * 1.05, text("Пик: $(round(peak_value, digits=1)) на $(round(peak_time, digits=1)) день", 8, :top))

# ### 5.3 Логарифмический масштаб (для анализа экспоненциального роста)
