...
#Получите уровень инфицирования β
β = params[:β_und][1]
...
peak_day = findfirst(==(peak_infected), I_vals)
initial_total = total_vals[1]
peak_pct = peak_infected / initial_total * 100
...
println("Пик эпидемии: $(round(peak_pct, digits=1))% населения")
...
vline!([peak_day], color = :black, linestyle = :dot, label = "Peak (day $peak_day)")

# Сохраняем график
savefig(plotsdir("sir_basic_dynamics_q1.png"))
println("\nГрафик сохранён")
